---
aliases:
tags:
- Kreatur/Humanoide/Goblinoide/Goblin
---
```statblock
image: [[goblin_terror_shaman.jpg]]
name: Goblin Schreckensschamane
size: [[Klein]]
type: [[Humanoide]]
subtype: ([[Goblins|Goblin]], [[Goblinoide]])
alignment: [[Neutral Böse]]
ac: 12
hp: 22
speed: 6
stats: [8, 14, 12, 10, 8, 14]
skillsaves:
  - Einschüchtern: 4
  - Heimlichkeit: 6
senses: [[Dunkelsicht]], [[Wahrnehmung#Passive Wahrnehmung]] 9
languages: Gemeinsprache, Goblinisch
cr: 1
spells:
  - Der Schreckensschamane ist ein Zauberwirker der dritten Stufe. Sein [[Zauberattribut]] ist [[Charisma]] 
    ([[Zauberrettungswurf-Schwierigkeitsgrad]] 12; [[Zauberangriffswürfe|Zauberangriffswurf]]-Modifikator +4).
    Er besitzt 2 [[Zauberplätze]] des 2. [[Zaubergrad|Grades]].
    Er regeneriert verbrauchte [[Zauberplätze]] bei einer [[Kurze Rast|Kurzen Rast]] oder [[Lange Rast|Langen Rast]].
  - Zaubertricks: [[Kalte Hand]], [[Schauriger Strahl]]
  - 1. Grad: [[Befehl]], [[Zeichen des Schreckens]]
  - 2. Grad: [[Finsternis]], [[Person festhalten]]
traits:
  - name: [[Mut stehlen]]
  - name: [[Blutmücken Vetrauter]]
actions:
  - name: [[Kampfstab]]
    desc: Nahkampf-Waffenangriff +4 zum Treffen, Reichweite 1,5m, ein Ziel. Treffer 1W8+2 Wuchtschaden
  - name: Blutmückenangriff
    desc: Der Blutmücken Vetraute darf einen Angriff durchführen indem er seine [[Zug#Reaktion]] opfert.
reactions:
  - name: Parieren
    desc: Der Kampfstab verleiht dem Goblin Schreckensschamanen die Eigenschaft [[Parieren]] als Reaktion.
```